  ...
  "fixed z-50 flex flex-col gap-4 bg-background p-4 md:p-6 shadow-lg transition ease-in-out overflow-y-auto",
  {
    variants: {
      side: {
        ...
        left: "inset-y-0 start-0 h-full w-[85vw] md:w-3/4 border-e data-[state=open]:animate-slide-in-from-left data-[state=closed]:animate-slide-out-to-left sm:max-w-sm ps-[calc(1rem+env(safe-area-inset-left))] md:ps-6",
        right:
          "inset-y-0 end-0 h-full w-[85vw] md:w-3/4 border-s data-[state=open]:animate-slide-in-from-right data-[state=closed]:animate-slide-out-to-right sm:max-w-sm pe-[calc(1rem+env(safe-area-inset-right))] md:pe-6",
      },
    },
    defaultVariants: {
      side: "right",
  ...
  <SheetPortal>
    ...
    >
      <DialogPrimitive.Close
        className="absolute end-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none min-h-[44px] min-w-[44px] flex items-center justify-center"
        aria-label="Close"
      >
        <svg
          aria-hidden="true"
