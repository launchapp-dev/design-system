...
const statusColors: Record<Payment["status"], string> = {
  ...
  pending:
    "text-yellow-600 bg-yellow-50 px-2 py-0.5 rounded-full text-xs font-medium",
  processing:
    "text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full text-xs font-medium",
...
const columns: ColumnDef<Payment>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <span
        className={statusColors[row.getValue("status") as Payment["status"]]}
      >
        {row.getValue("status")}
      </span>
    ),
